...
while len(heap) > 0:
    ...
    if row == len(lst) - 1 and col == len(lst[0]) - 1 and times_in_direction >= 4:
        print(heat)
        break
    if (row, col, row_movement, col_movement, times_in_direction) in seen:
        ...
        if times_in_direction < 10 and (row_movement, col_movement) != (0, 0):
        ...
        if times_in_direction >= 4 or (row_movement, col_movement) == (0,0):
